...
tag = subprocess.check_output(['git','describe','--tags','--exact-match'],cwd=root,text=True).strip()
assert tag == 'v0.1.1', 'This release job only adds assets to v0.1.1'
...
version = subprocess.check_output([binary,'--version'],text=True).strip(); assert version=='dcc 0.1.1', version
