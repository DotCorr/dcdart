// core/bench/benchmarks/hashmap-burst/bench.dart
//
// PHASE A of the `hashmap` pair. A DIAGNOSTIC, NOT A GATE INPUT.
//
//   hashmap-burst   phase A   bulk insert, bulk lookup, bulk teardown  <-- here
//   hashmap         phase B   the same work, interleaved
//
// Same map implementation (byte-identical between the two files, checked by
// `verify-parity.sh`), same keys, same operation counts, same checksum. The
// only difference is that this file BATCHES what the other INTERLEAVES.
//
// This phase exists to be beaten. It is built to be the allocator's best case
// and `malloc`'s worst: 1024 entries and 1024 values allocated back to back
// with nothing freed in between, then all of them freed before the next round.
// If DCDart wins here -- and `tree-traversal`, which has the same shape, won
// by 2.3x -- that is a measurement of ADR-0058's segregated size-class heap
// against `malloc`, not of ARC.
//
// `BENCH_SUITE=diagnostic`, so it never enters a geometric mean. The pair is
// the finding; averaging the two would produce a meaningless number precisely
// because the two effects point in opposite directions.
//
...
/// PHASE A -- BURST. Bulk insert, bulk lookup, bulk teardown.
///
/// Deliberately the allocator's BEST case and `malloc`'s worst: 1024 entries
/// and 1024 values allocated back to back with nothing freed in between, then
/// every one of them freed before the next round begins. DCDart bumps a
/// cursor through pre-zeroed `.bss`; `malloc` walks size bins, coalesces
/// neighbours on free and can return pages.
///
/// EXPECT DCDART TO WIN THIS ONE, and do not read the win as ARC being cheap.
/// `tree-traversal` has this same shape and came out 2.3x FASTER than C.
/// That is why this phase is `BENCH_SUITE=diagnostic` and is excluded from
/// every geometric mean, and why phase B -- the same work, interleaved -- is
/// the one that carries `BENCH_ID=hashmap` into the gate.
@bare u64 benchKernel(u64 rounds) {
  ...
  while (base < total) {
    ...
    while (j < u64(1024)) {
      final s = base + j;
      acc = (acc + mapRemove(root, s)) % u64(1000000007);
      j = j + u64(1);
    }
    base = base + u64(1024);
  }
