/* core/bench/benchmarks/hashmap-burst/kernel.c
 *
 * The C baseline for phase A (burst) of the `hashmap` pair.
 *
 * Everything above the kernel is byte-identical to `hashmap/kernel.c`
...
static uint64_t hashOf(uint64_t k) {
    uint64_t m = mod_ck(k, 1000003);
    uint64_t x = mul_ck(m, 2654435761);
    uint64_t h = (x >> 13) ^ (x >> 31);
    return h & 1023;
}

static uint64_t kindOf(uint64_t s, uint64_t total) {
    uint64_t era = div_ck(mul_ck(s, 3), total);
    uint64_t q = mod_ck(s, 10);
    if (era == 0) {
        if (q < 7) return 0;
        if (q < 9) return 1;
        return 2;
...
static struct Trie *buildTrie(uint64_t depth) {
    ...
    if (depth == 0) {
        ...
        n->c0 = buildTrie(sub_ck(depth, 1));
        n->c1 = buildTrie(sub_ck(depth, 1));
    }
    return n;
}
...
static uint64_t tinsert(struct Trie *n, uint64_t level, uint64_t h, struct Entry *e) {
    ...
    uint64_t bit = (h >> sub_ck(level, 1)) & 1;
    struct Trie *c = bit == 0 ? n->c0 : n->c1;
    if (c) return tinsert(c, sub_ck(level, 1), h, e);
    return 0;
}
...
static uint64_t valueSum(const struct Entry *e) {
    if (e->kind == 0) {
        ...
        if (v) return add_ck(v->a[0], v->a[2]);
        return 0;
    }
    if (e->kind == 1) {
        const struct VM *v = e->vm;
        if (v) return add_ck(v->a[0], v->a[11]);
        return 0;
    }
    const struct VB *v = e->vb;
    if (v) return add_ck(v->a[0], v->a[30]);
    return 0;
}
...
static uint64_t tlookup(const struct Trie *n, uint64_t level, uint64_t h, uint64_t key) {
    ...
    if (c) return tlookup(c, sub_ck(level, 1), h, key);
    return 0;
}
...
static uint64_t tremove(struct Trie *n, uint64_t level, uint64_t h, uint64_t key) {
    ...
    uint64_t bit = (h >> sub_ck(level, 1)) & 1;
    struct Trie *c = bit == 0 ? n->c0 : n->c1;
    if (c) return tremove(c, sub_ck(level, 1), h, key);
    return 0;
}

static uint64_t mapInsert(struct Trie *root, uint64_t key, uint64_t kind) {
    uint64_t v = mod_ck(key, 1000003);
...
uint64_t benchKernel(uint64_t rounds) {
    ...
    uint64_t total = mul_ck(rounds, 1024);
    uint64_t acc = 0;
    for (uint64_t base = 0; base < total; base = add_ck(base, 1024)) {
        for (uint64_t j = 0; j < 1024; j = add_ck(j, 1)) {
            uint64_t s = add_ck(base, j);
            acc = mod_ck(add_ck(acc, mapInsert(root, s, kindOf(s, total))), 1000000007);
        }
        for (uint64_t j = 0; j < 1024; j = add_ck(j, 1)) {
            uint64_t s = add_ck(base, j);
            acc = mod_ck(add_ck(acc, mapLookup(root, s)), 1000000007);
        }
        for (uint64_t j = 0; j < 1024; j = add_ck(j, 1)) {
            uint64_t s = add_ck(base, j);
            acc = mod_ck(add_ck(acc, mapRemove(root, s)), 1000000007);
        }
    }
    dropTrie(root);
    return acc;
