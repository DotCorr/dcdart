 ...
 *
 * `tree-traversal-malloc` with DCDart's TRAPPING arithmetic semantics --
 * malloc-per-node, NOT the arena. Generated from this directory's own
 * kernel.c rather than copied from `tree-traversal/`, and that distinction is
 * the whole point: a trap-matched baseline built from the ARENA kernel would
 * be compared against a MALLOC plain-C kernel, and the ~5x allocator-strategy
 * difference between them would be reported as the cost of trapping
 * arithmetic. I made exactly that mistake creating this directory; the
 * numbers (480 ms plain vs 91 ms "trap-matched") were absurd enough to catch
 * immediately, which is the only reason it did not survive.
 *
