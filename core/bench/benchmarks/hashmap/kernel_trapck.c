...
uint64_t benchKernel(uint64_t rounds) {
    ...
    uint64_t acc = 0;
    for (uint64_t s = 0; s < total; s = add_ck(s, 1)) {
        acc = mod_ck(add_ck(acc, mapInsert(root, s, kindOf(s, total))), 1000000007);
        acc = mod_ck(add_ck(acc, mapLookup(root, s)), 1000000007);
        if (1024 <= s) {
            acc = mod_ck(add_ck(acc, mapRemove(root, sub_ck(s, 1024))), 1000000007);
        }
    }
    for (uint64_t d = sub_ck(total, 1024); d < total; d = add_ck(d, 1)) {
        acc = mod_ck(add_ck(acc, mapRemove(root, d)), 1000000007);
    }
    dropTrie(root);
    return acc;
}
