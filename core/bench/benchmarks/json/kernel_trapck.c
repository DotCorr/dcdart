 ...
 *
 * `json` in C with DCDart's TRAPPING arithmetic semantics. THIS IS THE GATE
 * BASELINE (ADR-0059): the <= 10% bar is measured against this file, so it
 * isolates ARC and the allocator rather than also charging DCDart for
 ...
 *
 * (Original note, true of both files: the C baseline for `json`.)
 *
...
static struct JNode *parse_number(struct Parser *p) {
    ...
    while (p->pos < p->len) {
        unsigned char c = p->buf[p->pos];
        if (c < 48 || c > 57) break;
        v = mod_ck(add_ck(mul_ck(v, 10), sub_ck(c, 48)), 1000000007);
        p->pos++;
    }
...
static uint64_t walk(const struct JNode *n) {
    ...
    h = mod_ck(add_ck(mul_ck(h, 31), number), 1000000007);
    h = mod_ck(add_ck(mul_ck(h, 31), strlen_), 1000000007);
    if (n->kind == J_ARR && n->as.first) h = mod_ck(add_ck(h, walk(n->as.first)), 1000000007);
    if (n->next) h = mod_ck(add_ck(h, walk(n->next)), 1000000007);
    return h;
}
...
uint64_t benchKernel(uint64_t rounds) {
    ...
    for (uint64_t r = 0; r < rounds; r++) {
        ...
        acc = mod_ck(add_ck(acc, walk(root)), 1000000007);
        drop(root);
    }
    free(doc);
    return acc;
